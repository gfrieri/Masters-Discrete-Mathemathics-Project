...
class RegisterRequest(BaseModel):
    ...
    salt: str = Field(..., json_schema_extra={"example": "Base64(sal_unica)"})
    # El Verifier de la contraseña (e.g., hash + elemento público G^w)
    verifier: str = Field(..., json_schema_extra={"example": "Base64(datos_de_verificacion_V)"})

class RegisterResponse(BaseModel):
    """
    Respuesta simple de éxito para el registro.
    """
    message: str = Field(..., json_schema_extra={"example": "Registro de usuario exitoso."})
...
class PakeStartRequest(BaseModel):
    ...
    client_public_element: str = Field(..., json_schema_extra={"example": "Base64(elemento_publico_1)"})
...
class PakeStartResponse(BaseModel):
    ...
    session_id: str = Field(..., json_schema_extra={"example": "e1f2g3h4i5j6k7l8"})
    # Elemento público del servidor (e.g., X_B en la notación del paper)
    server_public_element: str = Field(..., json_schema_extra={"example": "Base64(elemento_publico_2)"})
...
class PakeCompleteRequest(BaseModel):
    ...
    client_proof: str = Field(..., json_schema_extra={"example": "Base64(prueba_de_autenticacion_M1)"})
...
class PakeCompleteResponse(BaseModel):
    ...
    server_proof: str = Field(..., json_schema_extra={"example": "Base64(prueba_de_autenticacion_M2)"})
    ...
    session_key_derived: str = Field(..., json_schema_extra={"example": "Base64(clave_de_sesion_K)"})
    message: str = Field(..., json_schema_extra={"example": "Intercambio de claves completado exitosamente."})